...
public class Report
{
    ...
    public static void generate() {
        ...
        try {
            htmlTemplate = new String(Files.readAllBytes(Paths.get(Config.pathToReportTemplate())), "UTF-8");
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        htmlTemplate = htmlTemplate.replace("@@BROWSER@@", Config.browser);
        htmlTemplate = htmlTemplate.replace("@@BROKEN_TESTS@@", Integer.toString(brokenTests));

        PrintWriter writer = null;
        try {
            writer = new PrintWriter(Config.pathToReportOutput(), "UTF-8");
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static String generateSidebarMenu() {
        ...
        // массив с уникальными именами
        // в каждом файле удалять брейпоин из имени. проверять есть ли это имя в массиве с уникальными именами. добавлять если нету
        ...
        for (File file : filesList)
        {
            ...
            testName = testName.substring(0, testName.indexOf(Config.breakpointDelimiter));
    ...
    private static String generateTestsContent() {
        ...
        for (String testName : uniqueTestNames)
        {
            ...
            int breakpointId = 0;
            for (String breakpoint : Config.breakpoints())
            {
                breakpointId++;
                ...
                if(fileExist(Config.pathToActual() + fileName)) {
            ...
            for (String breakpoint : Config.breakpoints())
            {
                ...
                if(fileExist(Config.pathToActual() + fileName))
                {
                    ...
                    breakpointTabsHtml+= "<img class='test_img' src='../" +  Config.pathToExpected() + Screenshoter.getRealName(testName, breakpoint, "png") + "' alt='image'>" + System.lineSeparator();
                    breakpointTabsHtml+= "</div>" + System.lineSeparator();

                    breakpointTabsHtml+= "<div class='w3-col "+ gridSize +" w3-center'>" + System.lineSeparator();
                    breakpointTabsHtml+= "<b>ACTUAL</b>" + System.lineSeparator();
                    breakpointTabsHtml+= "<br>" + System.lineSeparator();
                    breakpointTabsHtml+= "<img class='test_img' src='../" + Config.pathToActual() + Screenshoter.getRealName(testName, breakpoint, "png") + "' alt='image'>" + System.lineSeparator();
                    breakpointTabsHtml+= "</div>" + System.lineSeparator();
                    ...
                    if(isBreakpointFailed(fileName)) {
                        ...
                        breakpointTabsHtml+= "<br>" + System.lineSeparator();
                        breakpointTabsHtml+= "<img class='test_img' src='../" + Config.pathToDiff() + Screenshoter.getRealName(testName, breakpoint, "png") + "' alt='image'>" + System.lineSeparator();
                        breakpointTabsHtml+= "</div>" + System.lineSeparator();

                        breakpointTabsHtml+= "<div class='w3-col m3 w3-center'>" + System.lineSeparator();
                        breakpointTabsHtml+= "<b>GIF</b>" + System.lineSeparator();
                        breakpointTabsHtml+= "<br>" + System.lineSeparator();
                        breakpointTabsHtml+= "<img class='test_img' src='../" + Config.pathToGif() + Screenshoter.getRealName(testName, breakpoint, "gif") + "' alt='image'>" + System.lineSeparator();
                        breakpointTabsHtml+= "</div>" + System.lineSeparator();
                    }
    ...
    private static boolean isTestFailed(String testName) {
        ...
        for (String breakpoint : Config.breakpoints())
        {
            File f = new File(Config.pathToDiff() + Screenshoter.getRealName(testName, breakpoint, "png"));
    ...
    private static boolean isBreakpointFailed(String fileName)  {
        File f = new File(Config.pathToDiff() + fileName);
    ...
    private static String generateErrorsLog() {
        ...
        try {
            errorsLog = new String(Files.readAllBytes(Paths.get(Config.pathToErrorsLog())));
        } catch (IOException e) {
            e.printStackTrace();
        }
